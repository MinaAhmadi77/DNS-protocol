...
def decode_message(message):
    ...
    if int(ANCOUNT, 16) > 0:
        ...
        for ANSWER_COUNT in range(int(ANCOUNT, 16)):
            ...
            try:
                ...
                messages.append(ATYPE)
                ip_Answer.append(RDDATA_decoded)
                counter=counter+1
    if int(NSCOUNT, 16) > 0:
        for ANSWER_COUNT in range(int(NSCOUNT, 16)):
            if (ANSWER_SECTION_STARTS < len(message)):
                ANAME = message[ANSWER_SECTION_STARTS:ANSWER_SECTION_STARTS + 4]  # Refers to Question
                ATYPE = message[ANSWER_SECTION_STARTS + 4:ANSWER_SECTION_STARTS + 8]
                ACLASS = message[ANSWER_SECTION_STARTS + 8:ANSWER_SECTION_STARTS + 12]
            ...
            try:
               ...
               # counter = counter + 1
    if int(ARCOUNT, 16) > 0:
        for ANSWER_COUNT in range(int(ARCOUNT, 16)):
            if (ANSWER_SECTION_STARTS < len(message)):
                ANAME = message[ANSWER_SECTION_STARTS:ANSWER_SECTION_STARTS + 4]  # Refers to Question
                ATYPE = message[ANSWER_SECTION_STARTS + 4:ANSWER_SECTION_STARTS + 8]
                ACLASS = message[ANSWER_SECTION_STARTS + 8:ANSWER_SECTION_STARTS + 12]
...
f=decode_message(response)
m=0
while len(messages)>0:

    flag=True
    for i in range(len(messages)):
        if not messages[i]==get_type("A"):
            flag=False
    ...
    if flag==False:
        ...
        messages.clear()
        ip_Answer.clear()
        response = send_udp_message(message,m, 53)
        g=decode_message(response)
    else:
        print("\nResponse:\n" + response)
        print("\nResponse (decoded):" + decode_message(response))
        break